...
export default function VendorsEdit({ wedding, vendor, categories }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Edit', href: `/weddings/${wedding.id}/vendors/${vendor.id}/edit` },
    ];
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        final_payment_due_date: vendor.final_payment_due_date || '',
        final_payment_paid: vendor.final_payment_paid,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/weddings/${wedding.id}/vendors/${vendor.id}`);
    };

    return (
        <WeddingLayout breadcrumbs={breadcrumbs} wedding={wedding}>
            <Head title={`${wedding.title} - Edit ${vendor.name}`} />

            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="icon" asChild>
                        <Link href={`/weddings/${wedding.id}/vendors/${vendor.id}`}>
